...
def load_traindata():
    ...
    x_name = pd.read_csv("train_val.csv")['Id']   # 此处[' ']内填写的是train_val.csv内的第一列表头
    count_file = 0
...
'''
tr_set = np.array(load_traindata()).astype('float32')/255
tr_label = keras.utils.to_categorical(pd.read_csv("train_val.csv")['label'], 2)[0:495]

set_train, set_test, label_train, label_test = train_test_split(tr_set, tr_label, test_size=0.2, random_state=3)
set_train, label_train = data_augmentation(set_train, label_train, 0)
set_test, label_test = data_augmentation(set_test, label_test, 1)
set_train = set_train.reshape(set_train.shape[0], 32, 32, 32, 1)
set_test = set_test.reshape(set_test.shape[0], 32, 32, 32, 1)
...
model = createDenseNet(nb_classes=nb_classes, img_dim=[32, 32, 32, 1], depth=dense_depth,
                       growth_rate=dense_growthrate, dropout_rate=0.15)
model.compile(loss=categorical_crossentropy, optimizer=SGD(lr=0.001, decay=1e-5, momentum=0.85, nesterov=True),
              metrics=['accuracy'])
model.summary()
...
loss, accuracy = model.evaluate(set_test, label_test)
